...
module.exports = {
  ...
  devTool: 'inline-source-map',   //生成map 映射对应代码  方便错误查询
  devServer:{
    contentBase: './dist',    // 告诉服务从哪提供代码内容(静态文件这么使用)
    hot:true    //hot模式开启 
  },
  plugins:[
    new CleanWebpackPlugin(),  // 告诉清理插件的目录，output.path
    ...
    new VueLoaderPlugin()                 //vue-loader插件开启，解析.vue文件
  ],
  output: {        //出口
  ...
  resolve:{
        ...
        alias:{
            //正在使用的是vue的运行时版本，而此版本中的编译器是不可用的，我们需要把它切换成运行时 + 编译的版本
